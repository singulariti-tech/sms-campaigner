...
public class OutboundMessage extends Message {
    ...
    public String getPart(int partNo, int udhLength) {
        ...
        if (partNo != 0) {
            if (getEncoding() != MessageEncodings.ENCUCS2) {
                ...
                partSize = (maxSize() - 1) - (udhLength / 2);
            }
            partSize *= 2;
            if (((partSize * (partNo - 1)) + partSize) > getEncodedText().length()) {
                return getEncodedText().substring(partSize * (partNo - 1));
            } else {
                return getEncodedText().substring(partSize * (partNo - 1), (partSize * (partNo - 1)) + partSize);
            }
        } else {
    ...
    public Date getDispatchDate() {
        if (dispatchDate != null) {
            return new java.util.Date(dispatchDate.getTime());
        } else {
            return null;
        }
    }
