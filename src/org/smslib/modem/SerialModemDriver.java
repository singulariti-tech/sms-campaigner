...
class SerialModemDriver extends AModemDriver implements SerialPortEventListener {
    ...
    void connectPort() throws GatewayException, IOException, InterruptedException {
        if (gateway.getService().S.SERIAL_NOFLUSH) {
            gateway.logInfo("Comm port flushing is disabled.");
        }
        if (gateway.getService().S.SERIAL_POLLING) {
            gateway.logInfo("Using polled serial port mode.");
        }
        try {
            ...
            if (gateway.getService().S.SERIAL_RTSCTS_OUT) {
            ...
            if (gateway.getService().S.SERIAL_POLLING) {
                threadReader = new ThreadReader(gateway);
            }
