...
import org.smslib.AsyncEvents;
import org.smslib.GatewayException;
import org.smslib.GatewayStatuses;
...
public abstract class AModemDriver {
        ...
        synchronized (SYNC_Commander) {
            ...
            while (true) {
                ...
                if (response.indexOf("SIM PIN2") >= 0) {
                    ...
                    if ((gateway.getSimPin2() == null) || (gateway.getSimPin2().length() == 0)) {
                        throw new GatewayException("The GSM modem requires SIM PIN2 to operate.");
                    }
                    if (!gateway.getATHandler().enterPin(gateway.getSimPin2())) {
                    ...
                    if ((gateway.getSimPin() == null) || (gateway.getSimPin().length() == 0)) {
                        throw new GatewayException("The GSM modem requires SIM PIN to operate.");
                    }
                    if (!gateway.getATHandler().enterPin(gateway.getSimPin())) {
            ...
            if (!gateway.getATHandler().setIndications()) {
                gateway.logWarn("Callback indications were *not* set succesfully!");
            }
            if (gateway.getProtocol() == MessageProtocols.PDU) {
                if (!gateway.getATHandler().setPduProtocol()) {
                ...
                if (!gateway.getATHandler().setTextProtocol()) {
    ...
    public String getResponse() throws GatewayException, TimeoutException, IOException {
        ...
        try {
            while (true) {
                ...
                while (true) {
                    c = queue.get();
                    if (gateway.getService().S.DEBUG_QUEUE) {
                        gateway.logDebug("OUT READER QUEUE : " + (int) c + " / " + (char) c);
                    }
                    if (c != 0x0a) {
            ...
            if (i >= terminators.length - 4) {
                ...
                if ((event == AsyncEvents.INBOUNDMESSAGE) || (event == AsyncEvents.INBOUNDSTATUSREPORTMESSAGE) || (event == AsyncEvents.INBOUNDCALL)) {
                    asyncNotifier.setEvent(event, buffer.toString());
                }
                return getResponse();
            }
    ...
    boolean waitForNetworkRegistration() throws GatewayException, TimeoutException, IOException, InterruptedException {
        ...
        while (true) {
            ...
            switch (answer) {
                ...
                case 5:
                    gateway.logInfo("GSM: Registered to foreign network (roaming).");
                    return true;
                case -1:
    ...
    private class CharQueue {
        ...
        public synchronized void put(byte c) {
            ...
            buffer[bufferEnd] = c;
            bufferEnd++;
            if (bufferEnd == gateway.getService().S.SERIAL_BUFFER_SIZE) {
                bufferEnd = 0;
            }
            if (gateway.getService().S.DEBUG_QUEUE) {
                gateway.logDebug("IN READER QUEUE : " + (int) c + " / " + (char) c);
            }
            notifyAll();
        }
            ...
            while (true) {
                try {
                    if (bufferStart == bufferEnd) {
                        wait(gateway.getService().S.SERIAL_TIMEOUT);
                    }
                    if (bufferStart == bufferEnd) {
                    ...
                    bufferStart++;
                    if (bufferStart == gateway.getService().S.SERIAL_BUFFER_SIZE) {
                        bufferStart = 0;
                    }
                    return c;
                } catch (InterruptedException e) {
                    if (gateway.getStarted()) {
                        gateway.logWarn("Ignoring InterruptedException in Queue.get().");
                    } else {
        ...
        public synchronized byte peek() throws InterruptedException {
            ...
            while (true) {
                try {
                    ...
                    if (gateway.getStarted()) {
                        gateway.logWarn("Ignoring InterruptedException in Queue.peek().", e);
                    } else {
        ...
        public synchronized String peek(int sizeToRead) {
            ...
            while (size > 0) {
                ...
                i++;
                if (i == gateway.getService().S.SERIAL_BUFFER_SIZE) {
                    i = 0;
                }
                if (i == bufferEnd) {
                    break;
        ...
        public void run() {
            ...
            while (connected) {
                try {
                    ...
                    data = queue.peek(6);
                    if ((data.indexOf("CMTI") >= 0) || (data.indexOf("CDSI") >= 0) || (data.indexOf("RING") >= 0)) {
                        keepAlive.interrupt();
                    }
                } catch (InterruptedException e) {
                    if (!connected) {
            ...
            while (true) {
                try {
                    ...
                    synchronized (SYNC_Commander) {
                        ...
                        if (!gateway.getATHandler().isAlive()) {
    ...
    private class AsyncNotifier extends Thread {
        ...
        private org.smslib.helper.Queue eventQueue;
        private Object SYNC;

        public AsyncNotifier() {
            SYNC = new Object();
            eventQueue = new org.smslib.helper.Queue();
            setPriority(MIN_PRIORITY);
            start();
                ...
                try {
                    ...
                    if (event.event == AsyncEvents.INBOUNDMESSAGE) {
                        ...
                        if (gateway.getInboundNotification() != null) {
                            gateway.getInboundNotification().process(gateway.getGatewayId(), MessageTypes.INBOUND, getMemLoc(response), getMemIndex(response));
                        }
                        ...
                        response = event.response;
                        if (gateway.getInboundNotification() != null) {
                            gateway.getInboundNotification().process(gateway.getGatewayId(), MessageTypes.STATUSREPORT, getMemLoc(response), getMemIndex(response));
                        }
                        ...
                        if (gateway.getCallNotification() != null) {
                            gateway.getCallNotification().process(gateway.getGatewayId(), getOriginator(response));
                        }
                    }
                    //}
                } catch (InterruptedException e) {
