...
public class ModemGateway extends AGateway {
    ...
    ModemGateway(ModemTypes type, String id, String modemDevice, int modemParms, String manufacturer, String model) {
        ...
        if (type == ModemTypes.SERIAL) {
            driver = new SerialModemDriver(this, this.modemDevice + ":" + this.modemParms);
        } else {
            driver = new IPModemDriver(this, this.modemDevice + ":" + this.modemParms);
        }
    ...
    public void startGateway() throws TimeoutException, GatewayException, IOException, InterruptedException {
        logInfo("Starting gateway, using " + atHandler.getDescription() + " AT Handler.");
        driver.connect();
        super.startGateway();
    ...
    public void readMessages(List msgList, MessageClasses msgClass) throws TimeoutException, GatewayException, IOException, InterruptedException {
        synchronized (driver.SYNC_Commander) {
            if (protocol == MessageProtocols.PDU) {
                readMessagesPDU(msgList, msgClass, 0);
            } else if (protocol == MessageProtocols.TEXT) {
    ...
    public InboundMessage readMessage(String memLoc, int memIndex) throws TimeoutException, GatewayException, IOException, InterruptedException {
        ...
        synchronized (driver.SYNC_Commander) {
            ...
            for (int i = 0, n = msgList.size(); i < n; i++) {
                if ((((InboundMessage) msgList.get(i)).getMemIndex() == memIndex) && (((InboundMessage) msgList.get(i)).getMemLocation().equalsIgnoreCase(memLoc))) {
                    return (InboundMessage) msgList.get(i);
                }
            }
            return null;
        }
    ...
    private boolean sendMessageTEXT(OutboundMessage msg) throws TimeoutException, GatewayException, IOException, InterruptedException {
        ...
        refNo = atHandler.sendMessage(0, "", msg.getRecipient(), msg.getText());
        if (refNo >= 0) {
            msg.setGatewayId(gtwId);
            msg.setRefNo("" + refNo);
            msg.setDispatchDate(new Date());
