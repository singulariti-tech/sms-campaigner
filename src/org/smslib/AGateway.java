...
import org.smslib.helper.Queue;
...
public abstract class AGateway {
    ...
    public void setInbound(boolean value) {
        if ((attributes & GatewayAttributes.RECEIVE) != 0) {
            inbound = value;
        }
    }
    ...
    public void setOutbound(boolean value) {
        if ((attributes & GatewayAttributes.SEND) != 0) {
            outbound = value;
        }
    }
    ...
    public boolean queueMessage(OutboundMessage msg) {
        if (msg.getPriority() == MessagePriorities.LOW) {
            lowQ.add(msg);
        } else if (msg.getPriority() == MessagePriorities.NORMAL) {
            normalQ.add(msg);
        } else if (msg.getPriority() == MessagePriorities.HIGH) {
            highQ.add(msg);
        }
        return true;
    }
    ...
    int getQueueLoad(MessagePriorities priority) {
        if (priority == MessagePriorities.LOW) {
            return lowQ.size();
        } else if (priority == MessagePriorities.NORMAL) {
            return normalQ.size();
        } else if (priority == MessagePriorities.HIGH) {
            return highQ.size();
        } else {
            return 0;
        }
            ...
            try {
                if (started) {
                    while (true) {
                        ...
                        if ((!started) || (gatewayStatus != GatewayStatuses.OK)) {
                            break;
                        }
                        if (msg != null) {
                            if (!sendMessage(msg)) {
                            ...
                            } else if (getOutboundNotification() != null) {
                                getOutboundNotification().process(gtwId, msg);
                            }
                        }
                        msg = null;
